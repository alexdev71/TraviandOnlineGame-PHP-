...
$replace[88] = $database->getUserMSG($bbcode->get_string_between($input, '[player]','[/player]'));
//$replace[88] = "<a href=\"spieler.php?uid=".($user->getid($bbcode->get_string_between($input, '[player]','[/player]')))."\">$1</a>";

$input = preg_replace('/\[message\]/', '', $input);
$input = preg_replace('/\[\/message\]/', '', $input);

// Disable any links in messages
if($session->access == 2){
    $regex = "@(https?://([-\w\.]+[-\w])+(:\d+)?(/([\w/_\.#-]*(\?\S+)?[^\.\s])?).*$)@";
    $input = preg_replace($regex, ' <i><span style="font:red">link</span></i>', $input);
}
