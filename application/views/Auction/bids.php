...
<div class="silver"><img class="silver" src="img/x.gif">  <?php $now = $database->getAuctionSilver($session->uid); echo ($session->silver); ?> / <?php echo $session->silver+$now['silver']; ?> </div>
...
include_once "application/views/Auction/alt.php";
	...
	if(isset($_GET['a']) && $_GET['a']==$id){
        ...
        if($session->uid == $uid){ $bidvalue = $silver; }else{ $bidvalue = $silver+1; }
        $minimum = $silver+1;
...
if ($page != "1"){ $paginationDisplay .=  '<a class="first" href="?action=bids&page=1"><img src="img/x.gif"></a> ';}else{
$paginationDisplay .=  '<img src="img/x.gif" class="first disabled"> ';
}
if ($page != "1" && $page != $lastPage){$paginationDisplay .=  '<a class="previous" href="?action=bids&page=' . $previous . '"><img src="img/x.gif"></a>';}else{
